      ...
      <div class="absolute top-2 left-2">
        <button @click="showProfile(photo)"
          ...
          <img v-if="photo.user.image" :src="photo.user.image"
            class="w-[2.8rem] h-[2.8rem] 2xl:w-[3rem] 2xl:h-[3rem] rounded-full" />
          <UserCircleIcon v-else class="text-white w-[2.8rem] h-[2.8rem] 2xl:w-[3rem] 2xl:h-[3rem] rounded-full">
          </UserCircleIcon>
        </button>
      </div>
      <div class="absolute top-2 right-2 text-white text-right">
        ...
        <div class="mt-1 flex justify-end cursor-pointer" v-if="photo.uuid !== store.state.user.data.uuid">
          <button @click="deleteFavPhoto(photo.id)" v-if="favPhoto" class="tooltip tooltip-bottom tooltip-info"
            data-tip="Unfav">
            <div v-if="!loading">
              <svg class="w-9 h-9 text-primary hover:text-primary/70" viewBox="0 0 24 24">
                <path fill="currentColor"
                  d="M12,21.35L10.55,20.03C5.4,15.36 2,12.27 2,8.5C2,5.41 4.42,3 7.5,3C9.24,3 10.91,3.81 12,5.08C13.09,3.81 14.76,3 16.5,3C19.58,3 22,5.41 22,8.5C22,12.27 18.6,15.36 13.45,20.03L12,21.35Z" />
              </svg>
            </div>
            <div v-else>
              <svg class="w-9 h-9 animate-spin text-white" viewBox="0 0 24 24">
